...
def filefacets_object_to_node(graph, n_file_facets, file_information):
    ...
    file_ext = ext[1:]
    graph.add((n_file_facets, NS_RDF.type, NS_UCO_OBSERVABLE.FileFacet))
    graph.add(
        (
            n_file_facets,
    ...
    graph.add((n_file_facets, NS_UCO_OBSERVABLE.extension, rdflib.Literal(file_ext)))
    if "size" in file_information.keys():
        graph.add(
            (
                n_file_facets,
                NS_UCO_OBSERVABLE.sizeInBytes,
                rdflib.term.Literal(file_information["size"], datatype=NS_XSD.integer),
            )
        )
...
def filecontent_object_to_node(graph, n_content_facets, file_information):
    ...
    graph.add((n_content_facets, NS_RDF.type, NS_UCO_OBSERVABLE.ContentDataFacet))
    graph.add((n_content_facets, NS_UCO_OBSERVABLE.byteOrder, byte_order_facet))
    graph.add(
        (
            byte_order_facet,
            NS_RDF.type,
...
def raster_object_to_node(graph, controlled_dict, n_raster_facets, file_information):
    ...
    if "EXIF ExifImageLength" in controlled_dict.keys():
        graph.add(
            (
                ...
                rdflib.term.Literal(
                    int(float(str(controlled_dict["EXIF ExifImageLength"]))),
        ...
        graph.add(
            (
                ...
                NS_UCO_OBSERVABLE.pictureHeight,
                rdflib.term.Literal(
                    int(float(str(controlled_dict["ExifImageHeight"]))),
                    datatype=NS_XSD.integer,
                ),
            )
        )
    if "EXIF ExifImageWidth" in controlled_dict.keys():
        graph.add(
            (
                ...
                rdflib.term.Literal(
                    int(float(str(controlled_dict["EXIF ExifImageWidth"]))),
                    datatype=NS_XSD.integer,
                ),
            )
        )
        ...
        graph.add(
            (
                ...
                NS_UCO_OBSERVABLE.pictureWidth,
                rdflib.term.Literal(
                    int(float(str(controlled_dict["ExifImageWidth"]))),
                    datatype=NS_XSD.integer,
                ),
            )
        )
    if "EXIF CompressedBitsPerPixel" in controlled_dict.keys():
        graph.add(
            (
                ...
                rdflib.term.Literal(
                    int(float(str(controlled_dict["EXIF CompressedBitsPerPixel"]))),
                    datatype=NS_XSD.integer,
                ),
            )
        )
        ...
        graph.add(
            (
                ...
                NS_UCO_OBSERVABLE.bitsPerPixel,
                rdflib.term.Literal(
                    int(float(str(controlled_dict["CompressedBitsPerPixel"]))),
                    datatype=NS_XSD.integer,
                ),
            )
        )
    graph.add(
        (
            ...
            NS_RDFS.comment,
            rdflib.Literal(
                "Information represented here from exif information not from "
